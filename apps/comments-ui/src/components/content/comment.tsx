...
const CommentBody: React.FC<CommentBodyProps> = ({html, canQuoteInReply, className = '', isHighlighted, onQuoteInReply}) => {
    ...
    return (
        <div className={`mt mb-2 flex flex-row items-center gap-4 pr-4 ${className}`}>
            <div dangerouslySetInnerHTML={dangerouslySetInnerHTML} ref={contentRef} className="gh-comment-content text-md -mx-1 text-pretty rounded-md px-1 font-sans leading-normal text-neutral-900 [overflow-wrap:anywhere] sm:text-lg dark:text-white/85" data-testid="comment-content"/>
            {quoteSelection && (
