...
const Page: React.FC<{children: ReactNode}> = ({children}) => {
    return <>
        ...
        <div className="fixed top-0 left-0 flex size-full bg-grey-50 dark:bg-grey-975 dark:tablet:bg-[#101114]" id="admin-x-settings-content">
            {children}
        </div>
    </>;
};
...
const MainContent: React.FC = () => {
    ...
    if (isEditorUser(currentUser)) {
        return (
            <Page>
                <div className='flex-1 overflow-y-auto overscroll-y-contain bg-white dark:bg-grey-975' id="admin-x-settings-scroller">
    ...
    return (
        <Page>
            ...
            <div className="fixed inset-x-0 top-0 z-[35] max-w-[calc(100%-16px)] flex-1 basis-[320px] overscroll-y-contain bg-white p-8 tablet:relative tablet:inset-x-auto tablet:top-auto tablet:h-full tablet:overflow-y-scroll tablet:bg-grey-50 tablet:py-0 dark:bg-grey-975 dark:tablet:bg-[#101114]" id="admin-x-settings-sidebar-scroller">
                <div className="relative w-full">
                    <Sidebar />
                </div>
            </div>
            <div className="relative h-full flex-1 overflow-y-scroll overscroll-y-contain bg-white pt-12 tablet:basis-[800px] dark:bg-grey-975 dark:tablet:bg-black" id="admin-x-settings-scroller">
                <Settings />
            </div>
        </Page>
    );
