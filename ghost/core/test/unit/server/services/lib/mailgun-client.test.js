...
describe('MailgunClient', function () {
  ...
  describe('send()', function () {
    ...
    describe('per-recipient Message-Id', function () {
      ...
      // Extracts one field value from the multipart/form-data request body
      ...
      it('sends a per-recipient Message-Id header when requested', async function () {
        ...
        const shape = new RegExp(`^${EMAIL_ID}\\.[a-f0-9]{32}@domain\\.com$`);
        assert.match(first.message_id, shape);
        assert.match(second.message_id, shape);
        assert.notEqual(first.message_id, second.message_id);
        // existing variables are kept alongside the new one
