...
class VerificationTrigger {
     ...
     * @param {(content: {amountTriggered: number, threshold: number, method: string}) => Promise<void>} deps.sendVerificationWebhook Sends a webhook to the escalation service to confirm that customer needs to be verified
        ...
        if (!this._isVerified()) {
            ...
            if (!this._isVerificationRequired()) {
                ...
                if (this._shouldUseWebhookFlow()) {
                    ...
                    const triggerSource = method ?? source;
