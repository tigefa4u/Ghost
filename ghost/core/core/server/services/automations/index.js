 ...
 */
const init = ({domainEvents, apiUrl, schedulerAdapter, internalKeys}) => {
    if (_enqueuePollNow) {
        return;
    }

    _enqueuePollNow = () => domainEvents.dispatch(StartAutomationsPollEvent.create());
    ...
    domainEvents.subscribe(StartAutomationsPollEvent, oneAtATime(async () => {
        await poll({memberWelcomeEmailService, enqueueAnotherPollNow: _enqueuePollNow, enqueueAnotherPollAt: enqueuePollAt});
    }));
...
/**
 * Re-arm the poll chain. Any queued poll signed under the previous scheduler
 ...
 * `previousKey` is accepted for signature symmetry with the other
 * reschedulers but unused here.
 */
const rescheduleAll = () => {
    _enqueuePollNow?.();
};
