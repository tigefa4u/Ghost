...
interface GiftServiceDeps {
    ...
    signAdminToken: SignAdminToken | null;
...
export class GiftService {
    ...
    private scheduleReminder(gift: Gift): void {
        const {schedulerAdapter, schedulerIntegration, signAdminToken, urlJoin, apiUrl} = this.deps;

        if (!schedulerAdapter || !schedulerIntegration || !signAdminToken || !urlJoin || !apiUrl) {
            return;
        }
